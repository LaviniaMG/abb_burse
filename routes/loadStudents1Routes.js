...
const uploadDir = path.join(__dirname, '../uploads-students/bursa-excelenta-olimpica');
...
async function processData(data, res) {
    try {
        const tipBursa1 = await tipBursa.findOne({ where: { idBursa: 1 } });
            ...
            if (parseFloat(studentData.media) >= 5 &&
                ...
                await solicitari.findOrCreate({
                    where: {
                        idUtilizator: utilizatorRecord.idUtilizator,
                        idStatus: 1,
                        denumireBursa: "Bursă Excelență Olimpică / Internațională",
                        idBursa: 1
                    },
                    defaults: {
                        idUtilizator: utilizatorRecord.idUtilizator,

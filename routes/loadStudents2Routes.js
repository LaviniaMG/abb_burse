...
const uploadDir = path.join(__dirname, '../uploads-students/bursa-perf-rezultate-deosebite');
...
async function processData(data, res) {
    try {
        const tipBursa1 = await tipBursa.findOne({ where: { idBursa: 2 } });
            ...
            if (parseFloat(studentData.media) >= 5 &&
                ...
                await solicitari.findOrCreate({
                    where: {
                        idUtilizator: utilizatorRecord.idUtilizator,
                        idStatus: 1,
                        denumireBursa: "Bursă de Performanță pentru Rezultate Deosebite",
                        idBursa: 1
                    },
                    defaults: {
                        idUtilizator: utilizatorRecord.idUtilizator,

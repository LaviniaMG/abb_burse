...
router.get('/', async (req, res) => {
    try {
        const bursa = await tipBursa.findOne({ where: { idBursa: 10 } });
        if (!bursa) {
            return res.status(404).send({ message: 'Bursa nu a fost găsită' });
        }

        const solicitariBursa = await solicitari.findAll({ where: { idBursa: 10 } });
        ...
        for (const fileData of processedFiles) {
            ...
            const [updated] = await solicitari.update(
                ...
                { where: { DocumentURL: fileData.file, idBursa: 10 } }
            );
